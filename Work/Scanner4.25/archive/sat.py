...
for frame in cam.capture_continuous(rawCapture, format="bgr", use_video_port=True):
   ...
   # if cnts is not None:
    ...
    now = img.copy()
    cnts, hierarchy = cv2.findContours(maskB.copy(),cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(now,cnts,-1,(0,255,0),3)
    cv2.imwrite('/var/www/html/archive/now.jpg',now)
    fileNowTime.write(str(int(time.clock()-startTime)))                 
    if answer:
        cv2.imwrite('/var/www/html/archive/yellow.jpg',now)
        fileYellowTime.write(str(int(time.clock()-startTime)))
        fileHoleList.write('<div> Yellow Alert - '+str(time.clock()-startTime)+'</div>')   
    if answer==2:
        cv2.imwrite('/var/www/html/archive/red.jpg',now)
        fileRedTime.write(str(time.clock()-startTime))
        fileHoleList.write('<div> Red Alert - '+str(time.clock()-startTime)+'</div>')   
